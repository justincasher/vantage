...
DEFAULT_DB_PATH = APP_CONFIG.get("database", {}).get(
...
class ItemType(enum.Enum):
    ...
    def requires_proof(self) -> bool:
        ...
        return self in {
            ...
            ItemType.COROLLARY,
            ItemType.EXAMPLE,  # Examples might need verification/proof
        }
...
class ItemStatus(enum.Enum):
    ...
    LATEX_REJECTED_FINAL = (
        "LatexRejectedFinal"  # LaTeX rejected after max review cycles
    )

    # Lean Processing States
    PENDING_LEAN = (
        "PendingLean"  # Ready for Lean code generation (assumes LATEX_ACCEPTED?)
    )
    LEAN_GENERATION_IN_PROGRESS = "LeanGenInProgress"  # LLM is generating Lean code
    LEAN_VALIDATION_PENDING = (
        "LeanValidationPending"  # Lean generated, awaiting check_and_compile
    )
    LEAN_VALIDATION_IN_PROGRESS = (
        "LeanValidationInProgress"  # check_and_compile is running
    )
    ...
    # Other States
    CONJECTURE_STATED = (
        "Pending"  # Conjectures start as Pending, awaiting proof attempt
    )
...
class LatexLink:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LatexLink":
        ...
        if "link_type" not in data:
            data["link_type"] = "statement"
        try:
            return cls(**data)
        except TypeError as e:
            raise TypeError(f"Error creating LatexLink from dict: {e}") from e
...
class KBItem:
    ...
    topic: str = "General"
    plan_dependencies: List[str] = field(
        default_factory=list
    )  # Names of items needed for the proof plan
    dependencies: List[str] = field(
        default_factory=list
    )  # Names of items discovered during Lean compilation
    latex_links: List[LatexLink] = field(default_factory=list)

    # State & Metadata
    status: ItemStatus = ItemStatus.PENDING
    failure_count: int = 0  # General failure counter
    latex_review_feedback: Optional[str] = (
        None  # Store reviewer feedback if LaTeX (statement or proof) rejected
    )
    generation_prompt: Optional[str] = (
        None  # Store last prompt sent to LLM (for any generation task)
    )
    raw_ai_response: Optional[str] = None  # Store last raw response from LLM
    lean_error_log: Optional[str] = (
        None  # Store output from check_and_compile_item on failure
    )
    ...
    def from_db_dict(cls, data: Dict[str, Any]) -> "KBItem":
        ...
        try:
            ...
            latex_proof = (
                data.get("latex_proof") if item_type.requires_proof() else None
            )
            ...
            return cls(
                ...
                latex_links=[
                    LatexLink.from_dict(link_data)
                    for link_data in json.loads(data.get("latex_links") or "[]")
                ],
                status=status,
...
def initialize_database(db_path: Optional[str] = None):
    ...
    with get_db_connection(effective_path) as conn:  # Use effective_path here
        ...
        _add_column_if_not_exists(
            cursor, "kb_items", "plan_dependencies", "TEXT"
        )  # JSON list
        _add_column_if_not_exists(
            cursor, "kb_items", "latex_links", "TEXT"
        )  # JSON list of dicts
        _add_column_if_not_exists(
            cursor, "kb_items", "failure_count", "INTEGER", default_value=0
        )
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_kbitem_status ON kb_items (status);"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_kbitem_topic ON kb_items (topic);"
        )
        conn.commit()
...
async def save_kb_item(
    ...
    if should_generate_latex or should_generate_nl:
        try:
            ...
            warnings.warn(
                "kb_search module not found when needed for embedding generation.",
                ImportWarning,
            )
    ...
    if should_generate_latex or should_generate_nl:
        ...
        tasks.append(
            kb_search.generate_embedding(
                item.latex_statement, EMBEDDING_TASK_TYPE_DOCUMENT, client
            )  # Use latex_statement only
            if should_generate_latex
            else asyncio.sleep(0, result=None)  # Placeholder if no generation needed
        )
        # Create task for NL embedding generation or a dummy task
        tasks.append(
            kb_search.generate_embedding(
                item.description_nl, EMBEDDING_TASK_TYPE_DOCUMENT, client
            )
            if should_generate_nl
            else asyncio.sleep(0, result=None)  # Placeholder if no generation needed
        )
        try:
            ...
            if generated_latex_np is not None:
                ...
                logger.debug(
                    f"Generated LaTeX statement embedding for {item.unique_name}"
                )
