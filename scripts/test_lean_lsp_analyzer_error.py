...
try:
    SHARED_LIB_ROOT_PATH = pathlib.Path(LEAN_AUTOMATOR_SHARED_LIB_PATH_STR).resolve(strict=True) # Use strict=True to ensure it exists
...
async def main():
    ...
    try:
        ...
        resolved_lake_path_str = await loop.run_in_executor(None, shutil.which, LAKE_EXECUTABLE_PATH_STR)
        if resolved_lake_path_str:
            ...
            if is_file:
                ...
                logger.debug(f"'lean' not found relative to 'lake'. Checking PATH for 'lean'.")
                ...
                if lean_exe_from_path:
                    ...
                    logger.warning("Could not find 'lean' relative to 'lake' or in PATH. Using default 'lean'.")
                    lean_exe = "lean"
        else:
            logger.warning(f"Could not find '{LAKE_EXECUTABLE_PATH_STR}' executable. Checking PATH directly for 'lean'.")
            lean_exe_from_path = await loop.run_in_executor(None, shutil.which, "lean")
            if lean_exe_from_path:
                lean_exe = lean_exe_from_path
                logger.info(f"Found 'lean' executable in PATH: {lean_exe}")
            else:
                logger.warning("Could not find 'lake' or 'lean' in PATH. Using default 'lean'.")
                lean_exe = "lean"
    except Exception as path_err:
        logger.error(f"Unexpected error determining lean executable path: {path_err}. Using default 'lean'.")
        lean_exe = "lean"
    ...
    try:
        ...
        try:
            ...
            if await loop.run_in_executor(None, temp_file_abs_path.is_file):
                ...
                logger.warning(f"Temporary file {temp_file_abs_path} not found during cleanup (might be expected if analysis failed early).")
...
if __name__ == "__main__":
    ...
    if not os.path.exists("pyproject.toml") or not os.path.exists("src"):
         print("ERROR: This script must be run from the project root directory ('vantage').", file=sys.stderr)
         sys.exit(1)
