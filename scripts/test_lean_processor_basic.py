...
try:
    ...
    from src.lean_automator import lean_interaction # Import the lean interaction module
...
LAKE_EXECUTABLE_PATH = os.getenv("LAKE_EXECUTABLE_PATH", "lake") # Get lake path from env or default
LAKE_TIMEOUT_SECONDS = int(os.getenv("LAKE_TIMEOUT_SECONDS", "120")) # Get timeout from env or default
...
async def setup_test_data(db_path: str):
    ...
    for item in items_to_create:
        try:
            ...
            logger.info(f"Created dependency item entry: {item.unique_name} with status {item.status.name}")
...
""".strip()
    ...
    try:
        ...
        logger.info(f"Created target item entry: {target_item.unique_name} with status {target_item.status.name}")
    except Exception as e:
...
async def main():
    ...
    if not os.getenv("LEAN_AUTOMATOR_SHARED_LIB_PATH"):
        logger.critical("LEAN_AUTOMATOR_SHARED_LIB_PATH missing from environment variables.")
        return
    ...
    for axiom_name in AXIOM_NAMES:
        ...
        try:
            ...
            placed_successfully, message = await lean_interaction.check_and_compile_item(
            ...
            if placed_successfully:
                ...
                if axiom_item and axiom_item.status == ItemStatus.PROVEN:
                    logger.info(f"Axiom {axiom_name} successfully placed and compiled. Status: PROVEN.")
                elif axiom_item:
                     logger.warning(f"Axiom {axiom_name} check_and_compile returned success, but DB status is {axiom_item.status.name}.")
                     # Might still be okay if persistent build passed, but worth noting
                else:
                     logger.warning(f"Axiom {axiom_name} check_and_compile returned success, but could not re-fetch item from DB.")

            else:
                logger.error(f"Failed to place and compile axiom {axiom_name}. Reason: {message}")
                ...
                if axiom_item and axiom_item.lean_error_log:
                    logger.error(f"Lean Error Log for {axiom_name}:\n{axiom_item.lean_error_log}")
                break # Stop processing axioms if one fails

        except Exception as e:
            logger.exception(f"An unexpected error occurred during check_and_compile_item for axiom {axiom_name}: {e}")
            all_axioms_placed = False
            break
    ...
    if all_axioms_placed:
        ...
        logger.info(f"Running generate_and_verify_lean for target item: {TARGET_ITEM_NAME}")
        lean_success = False
        try:
            if lean_processor and hasattr(lean_processor, 'generate_and_verify_lean'):
                ...
                logger.info(f"generate_and_verify_lean finished for target. Result: {'Success' if lean_success else 'Failure'}")
            else:
                raise ImportError("lean_processor or generate_and_verify_lean not available.")
        except Exception as e:
            logger.exception(f"An error occurred during generate_and_verify_lean for target {TARGET_ITEM_NAME}: {e}")

    else:
        logger.error("--- Phase 2 Skipped: Not all axioms were successfully placed and compiled. ---")
    ...
    if final_target_item:
        ...
        logger.info(f"  Final Lean Code (len={len(final_target_item.lean_code or '')}):\n--- START ---\n{final_target_item.lean_code}\n--- END ---")
        if final_target_item.status == kb_storage.ItemStatus.LEAN_VALIDATION_FAILED and final_target_item.lean_error_log:
            logger.warning(f"  Lean Error Log:\n--- START ---\n{final_target_item.lean_error_log}\n--- END ---")
        elif final_target_item.lean_error_log:
             logger.debug(f"  Last Lean Error Log (present even on success?):\n{final_target_item.lean_error_log}")
    else:
        logger.error(f"Could not retrieve final state for target item {TARGET_ITEM_NAME}")
