...
async def test_db():
    ...
    items_to_save = [
        ...
        KBItem(
            unique_name="item_A_nl",
            item_type=ItemType.REMARK,
            description_nl="About apples",
            embedding_nl=BYTES_A,
        ),
        KBItem(
            unique_name="item_B_nl",
            item_type=ItemType.REMARK,
            description_nl="About red apples",
            embedding_nl=BYTES_B,
        ),
        KBItem(
            unique_name="item_C_nl",
            item_type=ItemType.REMARK,
            description_nl="About oranges",
            embedding_nl=BYTES_C,
        ),
        # Use NOTATION type for the latex item, does not require proof
        KBItem(
            unique_name="item_D_latex",
            item_type=ItemType.NOTATION,
            latex_statement="\\sum x",
            embedding_latex=BYTES_D,
        ),  # Changed latex_exposition to latex_statement
        KBItem(
            unique_name="item_E_nl",
            item_type=ItemType.REMARK,
            description_nl="About green apples",
            embedding_nl=BYTES_E,
        ),
        KBItem(
            unique_name="item_F_no_embed",
            item_type=ItemType.REMARK,
            description_nl="No embedding here",
        ),
    ]

    # Save items asynchronously (use client=None to prevent embed generation)
    save_tasks = [
        save_kb_item(item, client=None, db_path=db_path) for item in items_to_save
    ]
    ...
    try:
        ...
        if os.path.exists(db_path):
            os.remove(db_path)
            logger.debug(
                f"Cleaned up test database: {db_path}"
            )  # <-- Use defined logger
        else:
        ...
        logger.warning(
            f"Could not clean up test database '{db_path}': {e}"
        )  # <-- Use defined logger
...
def mock_generate_query_embedding(mocker):
    ...
    return mocker.patch(
        "lean_automator.kb.search.generate_embedding",
        return_value=mock_query_vec,
        autospec=True,
    )
...
class TestKBSearchIntegration:
    ...
    @pytest.mark.asyncio
    async def test_find_similar_items_nl_success(
        self, test_db, mock_generate_query_embedding, mock_gemini_client_instance
    ):
        ...
        assert result_names == expected_names, (
            f"Results order mismatch: Expected {expected_names}, Got {result_names}"
        )
        ...
        assert results[0][1] == pytest.approx(sim_a, abs=1e-4), (
            f"Score mismatch for A: Expected ~{sim_a:.4f}, Got {results[0][1]:.4f}"
        )
        assert results[1][1] == pytest.approx(sim_b, abs=1e-4), (
            f"Score mismatch for B: Expected ~{sim_b:.4f}, Got {results[1][1]:.4f}"
        )
        assert results[2][1] == pytest.approx(sim_e, abs=1e-4), (
            f"Score mismatch for E: Expected ~{sim_e:.4f}, Got {results[2][1]:.4f}"
        )
        ...
        assert (
            call_args[2] is mock_gemini_client_instance
        )  # Check client instance passed

    @pytest.mark.asyncio
    async def test_find_similar_items_latex_success(
        self, test_db, mock_generate_query_embedding, mock_gemini_client_instance
    ):
        ...
        assert len(results) == top_n, (
            f"Expected {top_n} result for latex search, got {len(results)}"
        )
        assert results[0][0].unique_name == "item_D_latex", (
            "Incorrect item found for latex search"
        )
        ...
        # Verify mock call
        mock_generate_query_embedding.assert_called_once_with(
            query, "RETRIEVAL_QUERY", mock_gemini_client_instance
        )
    ...
    async def test_find_similar_items_no_matches(
        ...
        norm_query = mock_query_vec_ortho / np.linalg.norm(
            mock_query_vec_ortho
        )  # [0,0,0,1]
        norm_vec_e = VEC_E / np.linalg.norm(VEC_E)
        expected_score_e = np.dot(norm_query, norm_vec_e)  # Should be approx 0.1240

        assert results[0][0].unique_name == "item_E_nl", (
            "Expected item E to be the top (least dissimilar) result"
        )
        assert results[0][1] == pytest.approx(expected_score_e, abs=1e-4), (
            "Top score for dissimilar query is unexpectedly high or low"
        )
        assert results[0][1] < 0.2, "Top score for dissimilar query should be low"

    @pytest.mark.asyncio
    async def test_find_similar_items_top_n_limit(
        self, test_db, mock_generate_query_embedding, mock_gemini_client_instance
    ):
        ...
        # --- Assertions ---
        assert len(results) == top_n, (
            f"Expected exactly {top_n} results, got {len(results)}"
        )
    ...
    @pytest.mark.asyncio
    async def test_find_similar_items_query_embed_fails(
        self, test_db, mock_generate_query_embedding, mock_gemini_client_instance
    ):
        ...
        with pytest.warns(UserWarning, match="Failed to generate query embedding"):
            results = await find_similar_items(
                query, search_field, client=mock_gemini_client_instance, db_path=test_db
            )
    ...
    @pytest.mark.asyncio
    async def test_find_similar_items_no_client(
        self, test_db, mock_generate_query_embedding
    ):
