...
try:
    from lean_automator.kb.storage import (
        # DEFAULT_DB_PATH removed as it was unused
        ItemStatus,
        ItemType,
        KBItem,
        get_kb_item_by_name,
    ...
    # REMOVED: Import of internal helper _generate_imports_for_target no longer needed
...
# --- Configuration ---
LAKE_EXEC_PATH = os.environ.get(
    "LAKE_EXECUTABLE_PATH", "lake"
)  # Match env var name used elsewhere
...
SHARED_LIB_PATH = (
    pathlib.Path(SHARED_LIB_PATH_STR).resolve() if SHARED_LIB_PATH_STR else None
)
...
def check_shared_lib_path():
    ...
    if not SHARED_LIB_PATH or not SHARED_LIB_PATH.is_dir():
        msg = (
            f"Shared library path '{SHARED_LIB_PATH_STR}' does not exist "
    ...
    if (
        ...
        warnings.warn(
            f"Shared library path {SHARED_LIB_PATH} seems to be missing a lakefile."
        )
    return True, ""
...
pytestmark = [
    ...
    pytest.mark.skipif(
        not is_lake_available(),
        reason=f"Lake executable '{LAKE_EXEC_PATH}' not found in PATH.",
    ),
    pytest.mark.skipif(
        not shared_lib_ok, reason=f"Shared library path issue: {shared_lib_reason}"
    ),
]
...
def get_expected_path(item_name: str) -> pathlib.Path:
    ...
    # Derive relative path from unique name, removing shared lib source dir prefix
    # if present
...
def cleanup_shared_lib(request):
    ...
    for parent_dir in sorted(cleaned_parents, key=lambda p: len(p.parts), reverse=True):
        ...
        if (
            parent_dir.is_relative_to(shared_lib_src_path)
            and parent_dir != shared_lib_src_path
        ):
            try:
...
@pytest.mark.asyncio
async def test_compile_success_no_deps_no_cache(
    test_db, monkeypatch, cleanup_shared_lib
):
    """Test compiling simple item directly into shared lib (no cache env var)."""
    monkeypatch.delenv(CACHE_ENV_VAR, raising=False)
    item_name = (
        f"{SHARED_LIB_SRC_DIR}.Test.NoDeps.NoCache"  # Ensure name includes lib prefix
    )
    ...
    item = KBItem(
        ...
        lean_code="def simpleDefNC : Nat := 5",
        topic="Test",
    ...
    assert expected_file_path.exists(), (
        "Lean file was not created/kept in shared lib on success."
    )
...
@pytest.mark.asyncio
async def test_compile_success_no_deps_with_cache(
    test_db, temp_cache_dir, monkeypatch, cleanup_shared_lib
):
    """Test compiling simple item directly into shared lib (with cache env var)."""
    monkeypatch.setenv(CACHE_ENV_VAR, temp_cache_dir)
    item_name = (
        f"{SHARED_LIB_SRC_DIR}.Test.NoDeps.WithCache"  # Ensure name includes lib prefix
    )
    expected_file_path = get_expected_path(item_name)
    ...
    item = KBItem(
        unique_name=item_name,
        item_type=ItemType.DEFINITION,
        lean_code="def simpleDefWC : Nat := 6",
        topic="Test",
        latex_statement="Dummy",
    )
    await save_kb_item(item, client=None, db_path=test_db)
...
@pytest.mark.asyncio
async def test_compile_success_with_deps_cached(
    test_db, temp_cache_dir, monkeypatch, cleanup_shared_lib
):
    ...
    dep_name = (
        f"{SHARED_LIB_SRC_DIR}.Test.DepA.Cached"  # Ensure name includes lib prefix
    )
    dep_path = get_expected_path(dep_name)
    cleanup_shared_lib(dep_path)
    dep_item = KBItem(
        unique_name=dep_name,
        item_type=ItemType.DEFINITION,
        lean_code="def depValueCached : Nat := 10",
        topic="Test",
        latex_statement="Dummy",
    )
    ...
    target_name = (
        f"{SHARED_LIB_SRC_DIR}.Test.TargetB.UsesCached"  # Ensure name includes prefix
    )
    ...
    assert target_path.exists(), (
        "Target Lean file was not created/kept in shared lib on success."
    )
...
async def test_compile_fail_syntax_error(
    test_db, temp_cache_dir, monkeypatch, use_cache, cleanup_shared_lib
):
    ...
    cache_str = "Cache" if use_cache else "NoCache"
    item_name = f"{SHARED_LIB_SRC_DIR}.Test.SyntaxErr.{cache_str}"  # Incl lib prefix
    expected_file_path = get_expected_path(item_name)
    ...
    assert not expected_file_path.exists(), (
        "Lean file with syntax error was not removed after failure."
    )
    assert "Lean validation failed" in message
    retrieved = get_kb_item_by_name(item_name, db_path=test_db)
    assert retrieved and retrieved.status == ItemStatus.LEAN_VALIDATION_FAILED
    assert retrieved.lean_error_log and (
        "error:" in retrieved.lean_error_log
        or "unknown identifier" in retrieved.lean_error_log
    )  # Check for common error patterns


@pytest.mark.asyncio
@pytest.mark.parametrize("use_cache", [False, True])
async def test_compile_fail_proof_error(
    test_db, temp_cache_dir, monkeypatch, use_cache, cleanup_shared_lib
):
    ...
    cache_str = "Cache" if use_cache else "NoCache"
    item_name = f"{SHARED_LIB_SRC_DIR}.Test.ProofErr.{cache_str}"  # Incl lib prefix
    expected_file_path = get_expected_path(item_name)
    ...
    item = KBItem(
        unique_name=item_name,
        item_type=ItemType.THEOREM,
        lean_code="theorem badProof : 1 + 1 = 3 := rfl",
        topic="Test",
        latex_statement="Dummy",
        latex_proof="Dummy",
    )
    ...
    assert success is False
    assert not expected_file_path.exists(), (
        "Lean file with proof error was not removed after failure."
    )
    assert "Lean validation failed" in message
    retrieved = get_kb_item_by_name(item_name, db_path=test_db)
    assert retrieved and retrieved.status == ItemStatus.LEAN_VALIDATION_FAILED
    assert retrieved.lean_error_log and (
        "error:" in retrieved.lean_error_log.lower()
        or "failed to synthesize" in retrieved.lean_error_log
    )  # Check for common proof errors


@pytest.mark.asyncio
@pytest.mark.parametrize("use_cache", [False, True])
async def test_compile_fail_missing_dependency_import(
    test_db, temp_cache_dir, monkeypatch, use_cache, cleanup_shared_lib
):
    ...
    cache_str = "Cache" if use_cache else "NoCache"
    item_name = f"{SHARED_LIB_SRC_DIR}.Test.MissingImport.{cache_str}"  # Incl prefix
    non_existent_dep_module = (
        f"{SHARED_LIB_SRC_DIR}.Test.DoesNotExistHopefully"  # Make name more unique
    )
    expected_file_path = get_expected_path(item_name)
    # No need to register for cleanup
    ...
    lean_code = (
        f"import {non_existent_dep_module}\n\ntheorem usesMissing : True := trivial"
    )
    item = KBItem(
        unique_name=item_name,
        item_type=ItemType.THEOREM,
        ...
        latex_statement="Dummy",
        latex_proof="Dummy",
        # Declaring plan_dependencies is good practice, but the import in
        # lean_code causes the failure
    ...
    assert success is False
    assert not expected_file_path.exists(), (
        "Lean file with missing import was not removed after failure."
    )
    assert "Lean validation failed" in message
    retrieved = get_kb_item_by_name(item_name, db_path=test_db)
    assert retrieved and retrieved.status == ItemStatus.LEAN_VALIDATION_FAILED
    ...
    assert (
        "unknown package" in retrieved.lean_error_log
        or "unknown module" in retrieved.lean_error_log
        or "could not resolve import" in retrieved.lean_error_log
        or "failed to build"
        in retrieved.lean_error_log  # Lake might report build failure
        or non_existent_dep_module in retrieved.lean_error_log
    ), f"Expected import error message not found in log:\n{retrieved.lean_error_log}"


@pytest.mark.asyncio
async def test_compile_success_dep_built_by_lake(
    test_db, monkeypatch, cleanup_shared_lib
):
    ...
    dep_name = f"{SHARED_LIB_SRC_DIR}.Test.DepNeedsBuilding.FromSource"  # Incl prefix
    dep_path = get_expected_path(dep_name)
    ...
    dep_item = KBItem(
        unique_name=dep_name,
        item_type=ItemType.DEFINITION,
        lean_code="def depValueNeedsBuildingFS : Nat := 30",
        topic="Test",
        latex_statement="Dummy",
    ...
    assert dep_success is True, (
        f"Dependency compilation failed unexpectedly: {dep_message}"
    )
    assert dep_path.exists(), (
        "Dependency .lean file should have been created by check_and_compile_item."
    )
    # Verify DB status for dependency is now PROVEN
    retrieved_dep_after_compile = get_kb_item_by_name(dep_name, db_path=test_db)
    assert (
        retrieved_dep_after_compile
        and retrieved_dep_after_compile.status == ItemStatus.PROVEN
    )
    print(
    ...
    target_name = (
        f"{SHARED_LIB_SRC_DIR}.Test.TargetUsesBuiltDep.FromSource"  # Incl prefix
    )
    target_path = get_expected_path(target_name)
    ...
    target_code = (
        f"import {dep_name}\n\n"
        f"theorem useBuiltDepFS : depValueNeedsBuildingFS = 30 := rfl"
    )
    target_item = KBItem(
        unique_name=target_name,
        item_type=ItemType.THEOREM,
    ...
    # --- Step 3: Compile the target; Lake should find and use the existing ---
    #             dependency source/build
    start_time = time.monotonic()
    target_success, target_message = await check_and_compile_item(
        target_name, db_path=test_db
    )
    end_time = time.monotonic()
    print(f"Target build time (dep built from source): {end_time - start_time:.4f}s")

    assert target_success is True, (
        f"Target compilation failed unexpectedly: {target_message}"
    )
    assert target_path.exists(), "Target lean file should exist on success."
    # Check dependency file still exists (it should)
    assert dep_path.exists(), (
        "Dependency file was unexpectedly removed during target compilation."
    )
...
async def test_compile_fail_lake_not_found(test_db, cleanup_shared_lib):
    ...
    item_name = f"{SHARED_LIB_SRC_DIR}.Test.LakeNotFound.Config"  # Include prefix
    ...
    expected_file_path = get_expected_path(item_name)
    assert not expected_file_path.exists(), (
        "Lean file should not exist after lake not found failure."
    )

    # Error message might come from LeanVerifier init or _run_lake_build
    assert (
        "Lake executable not found" in message
        or "No such file or directory" in message
        or "Failed to initialize" in message  # Could fail in init
        or "Configuration error" in message
    ...
    # If it fails during _run_lake_build (FileNotFoundError/RuntimeError),
    # status should be ERROR.
    assert retrieved.status in [ItemStatus.ERROR, ItemStatus.PENDING]
    if retrieved.status == ItemStatus.ERROR:
        assert retrieved.lean_error_log is not None
        assert (
            "Lake executable not found" in retrieved.lean_error_log
            or "No such file or directory" in retrieved.lean_error_log
        )


@pytest.mark.asyncio
async def test_compile_fail_shared_lib_path_invalid(test_db, monkeypatch):
    """Test fail when LEAN_AUTOMATOR_SHARED_LIB_PATH is invalid during Verifier init."""
    ...
    # Calling check_and_compile_item should now fail during LeanVerifier init
    success, message = await check_and_compile_item(item_name, db_path=test_db)

    assert success is False
    assert (
        "Configuration error" in message
    )  # Check specific error from check_and_compile_item wrapper
    assert (
        "Shared library path" in message or invalid_shared_lib_path in message
    )  # Check error originates from path validation
    assert "not configured correctly or the directory does not exist" in message

    retrieved = get_kb_item_by_name(item_name, db_path=test_db)
    assert retrieved is not None
    # Status should remain PENDING as verification couldn't even start due to
    # config error
...
async def test_compile_fail_timeout_mocked(test_db, mocker, cleanup_shared_lib):
    ...
    item_name = f"{SHARED_LIB_SRC_DIR}.Test.TimeoutMocked.Direct"  # Include lib prefix
    expected_file_path = get_expected_path(item_name)
    cleanup_shared_lib(expected_file_path)  # Register for cleanup
    ...
    mock_timeout_exception = subprocess.TimeoutExpired(
        cmd="mock lake build", timeout=short_timeout
    )
    mock_libdir_result = subprocess.CompletedProcess(
        args=["lean", "--print-libdir"],
        returncode=0,
        stdout="/mock/lean/lib",
        stderr="",
    )
    ...
    def mock_run_side_effect(*args, **kwargs):
        ...
        resolved_shared_lib_path = (
            str(SHARED_LIB_PATH.resolve()) if SHARED_LIB_PATH else None
        )
        ...
        is_correct_target = (
            command_args[-1] == target_module_name if command_args else False
        )
        is_correct_cwd = (
            str(command_cwd) == resolved_shared_lib_path
            if command_cwd and resolved_shared_lib_path
            else False
        )
        ...
        if is_build_command and is_correct_target and is_correct_cwd:
            # Debug print removed
            print(
                f"\nMock: Raising TimeoutExpired for build of '{target_module_name}' "
                f"in CWD '{command_cwd}'"
            )
        ...
        return subprocess.CompletedProcess(
            args=command_args, returncode=0, stdout=b"Mock output", stderr=b""
        )
    ...
    def which_side_effect(cmd):
        if cmd == "lean":
            return "/fake/path/to/lean"
        if cmd == LAKE_EXEC_PATH:
            return LAKE_EXEC_PATH  # Assume lake exists for this test
        return None
    ...
    print(
        f"\nCalling check_and_compile_item for {item_name} "
        f"with timeout {short_timeout}s"
    )
    success, message = await check_and_compile_item(
        item_name,
        db_path=test_db,
    ...
    assert success is False, (
        f"Expected compilation to fail due to timeout, but got success. "
        f"Message: {message}"
    )
    assert not expected_file_path.exists(), (
        f"Lean file '{expected_file_path}' should have been removed after "
        f"timeout failure."
    )
    ...
    for call in mock_subprocess_run.call_args_list:
        ...
        is_correct_cwd = (
            str(call_cwd) == resolved_shared_lib_path_str
            if call_cwd and resolved_shared_lib_path_str
            else False
        )
        ...
        if is_build_command and is_correct_target and is_correct_cwd:
            ...
            print(
                f"Verified mock call that timed out: args={args_list}, cwd={call_cwd}"
            )  # Debug
            break
    assert timeout_call_attempted, (
        f"Expected timeout during the direct build call ('{target_module_name}') "
        f"in shared lib ('{resolved_shared_lib_path_str}'). "
        f"Calls made: {mock_subprocess_run.call_args_list}"
    )

    # Assert DB state
    retrieved = get_kb_item_by_name(item_name, db_path=test_db)
    assert retrieved is not None, "Failed to retrieve item from DB after test."
    assert retrieved.status == ItemStatus.ERROR, (
        f"Expected status ERROR after timeout, but got {retrieved.status}."
    )
    assert retrieved.lean_error_log is not None, (
        "Expected lean_error_log to be set after timeout."
    )
    assert (
        "Timeout" in retrieved.lean_error_log or "timed out" in retrieved.lean_error_log
