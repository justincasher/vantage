...
def api_key() -> str:
    ...
    if not key:
        pytest.skip(
            "GEMINI_API_KEY environment variable not set. Skipping integration tests."
        )
...
class TestGeminiClientIntegration:
    ...
    async def test_successful_generation(self, client: GeminiClient):
        ...
        try:
            ...
            assert (
                ...
                or "integrate" in response.lower()
    ...
    async def test_explicit_generation_model_usage(self, client: GeminiClient):
        ...
        try:
            ...
            pytest.fail(
                f"Generation with explicit model '{TEST_GEN_MODEL}' failed: {e}"
            )
    ...
    async def test_generation_config_override(self, client: GeminiClient):
        ...
        try:
            response = await client.generate(
                prompt, generation_config_override=config_override
            )
            assert isinstance(response, str)
            assert len(response.split()) <= 2, (
                "Response should be very short due to max_output_tokens"
            )
            assert len(response) > 0, "Response should not be empty"
            # Simple check if it's likely a single word
            assert response.strip().isalnum(), (
                f"Response '{response}' doesn't look like a single word"
            )
        except Exception as e:
    ...
    async def test_safety_settings_override(self, client: GeminiClient):
        ...
        try:
            response = await client.generate(
                prompt, safety_settings_override=safety_override
            )
            assert isinstance(response, str)
    ...
    @pytest.mark.asyncio
    async def test_generation_cost_tracking_updates(
        ...
        initial_calls = (
            initial_summary.get("usage_by_model", {})
            .get(target_model, {})
            .get("calls", 0)
        )
        initial_input_units = (
        ...
        assert model_usage.get("calls", 0) == initial_calls + 1, (
            "Call count did not increment"
        )
        assert model_usage.get("input_units", 0) > initial_input_units, (
            "Input units did not increase"
        )
        assert model_usage.get("output_units", 0) > initial_output_units, (
            "Output units did not increase"
        )
    ...
    async def test_successful_embedding_single(self, client: GeminiClient):
        ...
        try:
            ...
            assert len(response) == 1, (
                "Response list should contain one embedding for single input"
            )
            assert isinstance(response[0], list), "Embedding should be a list of floats"
            assert len(response[0]) > 10, (
                "Embedding vector seems too short"
            )  # e.g., text-embedding-004 is 768
            assert all(isinstance(val, float) for val in response[0]), (
                "Embedding vector elements should be floats"
            )
        except Exception as e:
    ...
    async def test_successful_embedding_list(self, client: GeminiClient):
        ...
        try:
            ...
            assert isinstance(response, list), "Response should be a list"
            assert len(response) == len(contents), (
                f"Expected {len(contents)} embeddings, got {len(response)}"
            )  # Check length matches input list
            for i, vec in enumerate(response):
                ...
                assert all(isinstance(val, float) for val in vec), (
                    f"Embedding vector {i} elements should be floats"
                )
        except Exception as e:
    ...
    async def test_embedding_explicit_model_usage(self, client: GeminiClient):
        ...
        try:
            response = await client.embed_content(
                content, task_type, model=TEST_EMBED_MODEL
            )
            assert isinstance(response, list) and len(response) == 1, (
                "Response structure incorrect"
            )
            ...
            pytest.fail(
                f"Embedding with explicit model '{TEST_EMBED_MODEL}' failed: {e}"
            )

    @pytest.mark.asyncio
    async def test_invalid_embedding_model_name_raises_error(
        self, client: GeminiClient
    ):
    ...
    async def test_embedding_cost_tracking_updates(
        ...
        try:
            ...
            with warnings.catch_warnings(record=True) as caught_warnings:
                ...
                metadata_warning_found = any(
                    ...
                    and "Usage metadata not found" in str(w.message)
        ...
        final_output_units = model_usage.get(
            "output_units", 0
        )  # Should always be 0 for embeddings
        final_total_cost = final_summary.get("total_estimated_cost", 0.0)
        ...
        if metadata_warning_found:
            ...
            assert final_calls == initial_calls, (
                "Call count updated unexpectedly despite missing metadata"
            )
            assert final_input_units == initial_input_units, (
                "Input units updated unexpectedly despite missing metadata"
            )
            assert final_total_cost == pytest.approx(initial_total_cost), (
                "Total cost changed unexpectedly despite missing metadata"
            )
        else:
            ...
            assert final_input_units > initial_input_units, (
                "Input units did not increase (or metadata missing despite no warning)"
            )
