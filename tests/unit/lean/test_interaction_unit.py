...
try:
    ...
    from lean_automator.lean.interaction import logger as lean_interaction_logger
...
async def test_generate_imports_for_target_with_deps(mocker):
    item = KBItem(
        ...
        plan_dependencies=["VantageLib.MyLib.Dep.One", "VantageLib.MyLib.Dep.Two.Sub", "VantageLib.Test.Module.Item"],
        item_type=ItemType.THEOREM, description_nl="Desc", lean_code="code", topic="Topic"
    )
    mock_shared_path = pathlib.Path("/fake/valid/shared/lib")
    mocker.patch.object(LeanVerifier, '_resolve_shared_lib_path', return_value=mock_shared_path)
    mocker.patch("lean_automator.lean.interaction.get_lean_automator_shared_lib_path", return_value=str(mock_shared_path))
    mocker.patch("lean_automator.lean.interaction.APP_CONFIG", {"lean_paths": {"shared_lib_package_name": "VantageLib", "shared_lib_src_dir_name": "VantageLib"}})
...
async def test_generate_imports_for_target_no_deps(mocker):
    item = KBItem(unique_name="VantageLib.Another.Item", plan_dependencies=[],
                  item_type=ItemType.THEOREM, description_nl="Desc", lean_code="code", topic="Topic")
    mock_shared_path = pathlib.Path("/fake/valid/shared/lib")
    mocker.patch.object(LeanVerifier, '_resolve_shared_lib_path', return_value=mock_shared_path)
    mocker.patch("lean_automator.lean.interaction.get_lean_automator_shared_lib_path", return_value=str(mock_shared_path))
...
def patch_verifier_dependencies(mocker, mock_subprocess_result):
    ...
    mock_get = mocker.patch(
        "lean_automator.lean.interaction.get_kb_item_by_name",
        return_value=None
    )
    mock_save = mocker.patch(
        "lean_automator.lean.interaction.save_kb_item",
    ...
    mock_shared_path = pathlib.Path("/fake/valid/shared/lib")
    mocker.patch.object(lean_interaction_module.LeanVerifier, '_resolve_shared_lib_path', return_value=mock_shared_path)
    mocker.patch("lean_automator.lean.interaction.get_lean_automator_shared_lib_path", return_value=str(mock_shared_path))
    mocker.patch("lean_automator.lean.interaction.APP_CONFIG", {"lean_paths": {"shared_lib_package_name": "VantageLib", "shared_lib_src_dir_name": "VantageLib"}})
...
@pytest.mark.asyncio
async def test_check_and_compile_item_success(patch_verifier_dependencies, mock_kb_item):
    ...
    # mock_subprocess_run.return_value = mock_subprocess_result(returncode=0) # Already default

    success, message = await check_and_compile_item(mock_kb_item.unique_name)

    assert success is True
    assert "verified and integrated" in message
    mock_get_item.assert_called_once_with(mock_kb_item.unique_name, db_path=DEFAULT_DB_PATH)
    ...
    mock_subprocess_run.assert_any_call(
        ...
        capture_output=True, text=True, timeout=120, encoding='utf-8', errors='replace', check=False
    )
...
@pytest.mark.asyncio
async def test_check_and_compile_item_build_fails(patch_verifier_dependencies, mock_kb_item, mock_subprocess_result):
    mock_get_item = patch_verifier_dependencies["get_item"]
    mock_save_item = patch_verifier_dependencies["save_item"]
    mock_subprocess_run = patch_verifier_dependencies["subprocess_run"]
    mock_os_remove = patch_verifier_dependencies["os_remove"]
    ...
    # If using side_effect in fixture, configure it there. Otherwise, set return_value directly.
    mock_subprocess_run.return_value = mock_subprocess_result(returncode=1, stderr="Lean build error!")
...
@pytest.mark.asyncio
async def test_check_and_compile_item_write_fails(patch_verifier_dependencies, mock_kb_item):
    mock_get_item = patch_verifier_dependencies["get_item"]
    mock_save_item = patch_verifier_dependencies["save_item"]
    mock_subprocess_run = patch_verifier_dependencies["subprocess_run"]
    mock_os_remove = patch_verifier_dependencies["os_remove"]
...
@pytest.mark.asyncio
async def test_check_and_compile_item_timeout(patch_verifier_dependencies, mock_kb_item):
    mock_get_item = patch_verifier_dependencies["get_item"]
    mock_save_item = patch_verifier_dependencies["save_item"]
    mock_subprocess_run = patch_verifier_dependencies["subprocess_run"]
    mock_os_remove = patch_verifier_dependencies["os_remove"]
    ...
    mock_subprocess_run.side_effect = subprocess.TimeoutExpired(cmd="mock lake build", timeout=10)
    ...
    assert "timed out after" in saved_item.lean_error_log # Check for content that is actually logged

    mock_os_remove.assert_called_once()

@pytest.mark.asyncio
async def test_check_and_compile_item_lsp_analysis_fails(patch_verifier_dependencies, mock_kb_item, mock_subprocess_result):
    mock_get_item = patch_verifier_dependencies["get_item"]
    mock_save_item = patch_verifier_dependencies["save_item"]
    mock_subprocess_run = patch_verifier_dependencies["subprocess_run"]
    mock_os_remove = patch_verifier_dependencies["os_remove"]
...
async def test_check_and_compile_item_config_error(mocker):
    ...
    mocker.patch.object(lean_interaction_module.LeanVerifier, '_resolve_shared_lib_path', return_value=None)
    mocker.patch("lean_automator.lean.interaction.get_lean_automator_shared_lib_path", return_value=None)
