...
def mock_kb_items_for_search() -> List[Tuple[int, str, bytes]]:
    ...
    return [
        (1, "item_A", create_embedding_bytes([1.0, 0.0, 0.0])), # Vector pointing along X
        (2, "item_B", create_embedding_bytes([0.0, 1.0, 0.0])), # Vector pointing along Y
        (3, "item_C", create_embedding_bytes([-1.0, 0.0, 0.0])),# Vector opposite to A
        (4, "item_D", create_embedding_bytes([0.707, 0.707, 0.0])),# Vector between A and B
...
def mock_full_kb_items() -> dict:
    ...
    return {
        "item_A": KBItem(id=1, unique_name="item_A", item_type=ItemType.THEOREM, lean_code="A code", embedding_nl=create_embedding_bytes([1.0, 0.0, 0.0])),
        "item_B": KBItem(id=2, unique_name="item_B", item_type=ItemType.DEFINITION, lean_code="B code", embedding_nl=create_embedding_bytes([0.0, 1.0, 0.0])),
        "item_C": KBItem(id=3, unique_name="item_C", item_type=ItemType.LEMMA, lean_code="C code", embedding_nl=create_embedding_bytes([-1.0, 0.0, 0.0])),
        "item_D": KBItem(id=4, unique_name="item_D", item_type=ItemType.THEOREM, lean_code="D code", embedding_nl=create_embedding_bytes([0.707, 0.707, 0.0])),
...
async def test_generate_embedding_success(mock_gemini_client):
    ...
    mock_gemini_client.embed_content.assert_awaited_once_with(contents=text, task_type=task_type)
...
async def test_generate_embedding_empty_text(mock_gemini_client):
    ...
    with pytest.warns(UserWarning, match="Attempted to generate embedding for empty text"):
...
@pytest.mark.filterwarnings("ignore:Could not decode embedding for item 'item_E_bad_bytes'")
...
async def test_find_similar_items_success(mock_gemini_client, patch_db_functions):
    ...
    mock_gemini_client.embed_content.return_value = query_vector_list # Mock embedding generation

    top_n = 3
    results = await find_similar_items(query_text, search_field, mock_gemini_client, top_n=top_n, db_path=TEST_DB_PATH)
    ...
    mock_get_all.assert_called_once_with(f"embedding_{search_field}", TEST_DB_PATH or DEFAULT_DB_PATH)
    # Check get_kb_item_by_name called for the top N unique names found
    expected_get_calls = [call("item_A", TEST_DB_PATH),
                          call("item_D", TEST_DB_PATH),
                          call("item_B", TEST_DB_PATH)]
    mock_get_by_name.assert_has_calls(expected_get_calls, any_order=True) # Order depends on sorting


@pytest.mark.asyncio
async def test_find_similar_items_no_embeddings_in_db(mock_gemini_client, patch_db_functions):
    ...
    mock_gemini_client.embed_content.return_value = [[0.1, 0.2, 0.3]] # Query gen succeeds
...
@pytest.mark.asyncio
async def test_find_similar_items_query_embedding_fails(mock_gemini_client, patch_db_functions):
...
@pytest.mark.asyncio
async def test_find_similar_items_db_fetch_fails(mock_gemini_client, patch_db_functions):
...
@pytest.mark.asyncio
async def test_find_similar_items_handles_bad_bytes_and_dim_mismatch(mock_gemini_client, patch_db_functions):
    ...
    mock_gemini_client.embed_content.return_value = [[1.0, 0.0, 0.0]] # Query vector matches dim=3
    ...
    assert any("Could not decode embedding for item 'item_E_bad_bytes'" in msg for msg in warning_messages)
    assert any("Dimension mismatch for item 'item_F_wrong_dim'" in msg for msg in warning_messages)
...
@pytest.mark.filterwarnings("ignore:Dimension mismatch for item 'item_F_wrong_dim'")
@pytest.mark.asyncio
async def test_find_similar_items_handles_item_retrieval_fail(mock_gemini_client, patch_db_functions):
