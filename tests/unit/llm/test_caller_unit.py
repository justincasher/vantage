...
VALID_COST_JSON = json.dumps(
    {
        "model-a": {
            "input": 0.5,
            "output": 1.5,
        },  # Cost per Million Tokens (Generation)
        "model-b": {"input": 1.0, "output": 2.0},  # (Generation)
        "models/text-embedding-004": {
            "input": 0.1
        },  # Embedding model, output cost defaults to 0
    }
)
VALID_COST_JSON_EMBED_EXPLICIT_ZERO = json.dumps(
    {
        "models/text-embedding-004": {
            "input": 0.1,
            "output": 0.0,
        }  # Explicit zero output cost
    }
)
INVALID_COST_JSON = (
    '{"model-a": {"input": 0.5, "output": 1.5}, "model-b": }'  # Malformed JSON
)
...
class MockContent:
    ...
    def text(self) -> Optional[str]:
        if self.parts and hasattr(self.parts[0], "text"):
...
@dataclass
class MockCandidate:
    content: MockContent = field(
        default_factory=lambda: MockContent(parts=[MockPart()])
    )
...
class MockGenAIResponse:
    ...
    @property
    def text(self) -> Optional[str]:
        if self._text_manually_set:
            return self._text
        try:
            if self.candidates and self.candidates[0].text is not None:
    ...
    def text(self, value: Optional[str]):
        ...
        if value is not None:
            if not self.candidates:
                self.candidates = [
                    MockCandidate(content=MockContent(parts=[MockPart(text=value)]))
                ]
            else:
                if not self.candidates[0].content.parts:
                    self.candidates[0].content.parts = [MockPart(text=value)]
                else:
                    if hasattr(self.candidates[0].content.parts[0], "text"):
                        self.candidates[0].content.parts[0].text = value

    def __post_init__(self):
        if self.usage_metadata:
            self.usage_metadata.total_token_count = (
                self.usage_metadata.prompt_token_count or 0
            ) + (self.usage_metadata.candidates_token_count or 0)
...
def test_cost_tracker_init_valid_with_embedding():
    ...
    assert (
        tracker._model_costs[MOCK_DEFAULT_EMBED_MODEL].input_cost_per_million_units
        == 0.1
    )
    # Output cost should default to 0.0 if not specified in JSON
    assert (
        tracker._model_costs[MOCK_DEFAULT_EMBED_MODEL].output_cost_per_million_units
        == 0.0
    )
...
def test_cost_tracker_init_valid_embed_explicit_zero():
    ...
    tracker = GeminiCostTracker(
        model_costs_override=json.loads(VALID_COST_JSON_EMBED_EXPLICIT_ZERO)
    )
    assert MOCK_DEFAULT_EMBED_MODEL in tracker._model_costs
    assert (
        tracker._model_costs[MOCK_DEFAULT_EMBED_MODEL].input_cost_per_million_units
...
def test_cost_tracker_init_invalid_json():
    ...
    with pytest.warns(
        UserWarning, match="Invalid cost format for model 'invalid_structure'"
    ):
...
def test_cost_tracker_init_incomplete_json():
    ...
    assert (
        not tracker._model_costs
    )  # Expect empty cost dict if all entries fail validation
    warning_messages = [str(w.message) for w in record]
    # Check that warnings were issued for each problematic entry
    assert any(
        "Invalid cost format for model 'model-a'" in msg for msg in warning_messages
    )  # Missing 'input'
    assert any(
        "Invalid cost format for model 'model-c'" in msg for msg in warning_messages
    )  # Wrong keys
...
def test_cost_tracker_record_usage_single_embed():
    ...
    tracker.record_usage(
        MOCK_DEFAULT_EMBED_MODEL, 500, 0
    )  # Input units, 0 output units
...
def test_cost_tracker_get_total_cost_missing_model():
    ...
    tracker = GeminiCostTracker(
        model_costs_override=json.loads(VALID_COST_JSON)
    )  # Costs for A, B, embedding
    tracker.record_usage("model-a", 1_000_000, 0)  # Cost = 0.5
    tracker.record_usage("model-z", 500_000, 500_000)  # No cost info

    with pytest.warns(
        UserWarning, match="Cost information missing for model 'model-z'"
    ):
...
def test_cost_tracker_get_summary_with_embed():
    ...
    assert summary["total_estimated_cost"] == pytest.approx(
        expected_cost_a + expected_cost_embed
    )
...
def mock_config_and_key(mocker):
    ...
    mocker.patch(
        "lean_automator.llm.caller.APP_CONFIG", mock_config, create=True
    )  # Use create=True if APP_CONFIG might not exist yet

    # Mock the API key getter function
    mocker.patch(
        "lean_automator.llm.caller.get_gemini_api_key", return_value=MOCK_API_KEY
    )
...
def mock_genai_lib(mocker):
    ...
    mock_response_gen = MockGenAIResponse(
        ...
        candidates=[
            MockCandidate(
                content=MockContent(parts=[MockPart(text="Default Mock Response")])
            )
        ],
    )
    ...
    mock_response_embed = {
        ...
        "usage_metadata": {
            "total_token_count": 5
        },  # Assume metadata exists for base tests
    }
    ...
    mocker.patch(
        "lean_automator.llm.caller.google_api_exceptions.ResourceExhausted",
        google_api_exceptions.ResourceExhausted,
    )
    mocker.patch(
        "lean_automator.llm.caller.google_api_exceptions.GoogleAPIError",
...
def patch_asyncio_to_thread(mocker):
    ...
    async def run_sync_wrapper(func, *args, **kwargs):
...
def test_client_init_success(
    ...
    assert (
        client.default_embedding_model == MOCK_DEFAULT_EMBED_MODEL
...
def test_client_init_missing_api_key(
    mocker, mock_genai_lib
):  # Doesn't need base config fixture
...
def test_client_init_missing_default_embed_model_uses_fallback(
    ...
    base_config = mock_config_and_key.copy()  # Use copy
    if (
        "embedding" in base_config
        and "default_embedding_model" in base_config["embedding"]
    ):
...
def test_client_init_embed_model_adds_prefix(
    ...
    base_config = mock_config_and_key.copy()  # Use copy
    base_config["embedding"]["default_embedding_model"] = (
        embed_model_no_prefix  # Set value without prefix
    )
    mocker.patch("lean_automator.llm.caller.APP_CONFIG", base_config)
    # Key getter is already mocked
...
def test_client_init_with_args(
    mocker, mock_genai_lib
):  # Doesn't need config fixture (args override)
    ...
    mocker.patch(
        "lean_automator.llm.caller.get_gemini_api_key",
        return_value="DUMMY_KEY_FROM_GETTER",
    )
    mocker.patch(
        "lean_automator.llm.caller.APP_CONFIG",
        {"llm": {"default_gemini_model": "dummy-gen-model-config"}},
        create=True,
    )
...
@pytest.mark.asyncio
async def test_client_generate_success_default_model(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    mock_genai_module.GenerativeModel.assert_called_once_with(
        MOCK_DEFAULT_GEN_MODEL, system_instruction=None
    ...
    mock_record_usage.assert_called_once_with(
        client.cost_tracker, MOCK_DEFAULT_GEN_MODEL, 10, 20
    )
    mock_asyncio_sleep.assert_not_called()


@pytest.mark.asyncio
async def test_client_generate_with_system_prompt(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    # This assertion should now pass
    mock_genai_module.GenerativeModel.assert_called_once_with(
        MOCK_DEFAULT_GEN_MODEL, system_instruction=system_p
    )
...
@pytest.mark.asyncio
async def test_client_embed_success_single_string(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
...
@pytest.mark.asyncio
async def test_client_embed_success_list_strings(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    mock_embed_func.assert_called_once_with(
        model=MOCK_DEFAULT_EMBED_MODEL, content=contents_to_embed, task_type=task_type
    )

    # Verify cost tracker
    mock_record_usage.assert_called_once_with(
        client.cost_tracker, MOCK_DEFAULT_EMBED_MODEL, 15, 0
    )
    mock_asyncio_sleep.assert_not_called()


@pytest.mark.asyncio
async def test_client_embed_with_optional_args(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    await client.embed_content(
        content, task, title=doc_title, output_dimensionality=dims
    )
...
@pytest.mark.asyncio
async def test_client_embed_title_wrong_task_warns(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    assert any(
        "Ignoring 'title' argument as task_type is 'RETRIEVAL_QUERY'" in msg
...
@pytest.mark.asyncio
async def test_client_embed_model_needs_prefix(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    assert any(
        f"Embedding model name '{model_no_prefix}' should ideally start with 'models/'"
        in msg
        for msg in warning_messages
    )
...
@pytest.mark.asyncio
async def test_client_embed_retry_logic(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
    ...
    )  # Uses default models from mock_config_and_key

    expected_vector = [0.8, 0.9]
    mock_response_success = {
        "embedding": expected_vector,
        "usage_metadata": {"total_token_count": 6},
    }
    api_error = google_api_exceptions.ResourceExhausted(
        "Simulated Quota Error"
    )  # Use a specific retryable error
    ...
    mock_record_usage.assert_called_once_with(
        client.cost_tracker, MOCK_DEFAULT_EMBED_MODEL, 6, 0
    )
    ...
    assert any(
        f"API Quota/Rate Limit Error for {MOCK_DEFAULT_EMBED_MODEL} on attempt 1/2"
        in msg
        for msg in warning_messages
    )
    retry_msg = (
...
@pytest.mark.asyncio
async def test_client_embed_all_retries_fail(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
...
@pytest.mark.asyncio
async def test_client_embed_invalid_response_format(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
...
@pytest.mark.asyncio
async def test_client_embed_input_type_validation(
    mocker, mock_config_and_key, mock_genai_lib
):  # Use fixture
    ...
    with pytest.raises(
        TypeError, match="Input 'contents' must be a string or a list of strings"
    ):
        await client.embed_content(123, "RETRIEVAL_QUERY")  # Invalid type

    with pytest.raises(
        TypeError,
        match="If 'contents' is provided as a list, all its items must be strings",
    ):
        await client.embed_content(
            ["a string", 123], "RETRIEVAL_QUERY"
...
@pytest.mark.asyncio
async def test_client_embed_missing_usage_metadata(
    mocker,
    mock_config_and_key,
    mock_genai_lib,
    mock_asyncio_sleep,
